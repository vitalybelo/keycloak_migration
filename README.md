...
Работа методов проверена на версиях Keycloak 26.1.* - 26.4.*
...
### 🌍 6. Realm Configuration (Настройки области сервисов)

Глобальный экспорт и импорт настроек Realm (компоненты, провайдеры, политики) с возможностью инкапсулированного "частичного импорта" связанных сущностей в рамках одной транзакции.

* **Экспорт (`GET /configuration`)**
  * **Права:** `clients().requireView()`
  * **Параметры URL (все опциональные, тип Boolean):** 
    * `isMigrateRealmRoles` — включить в выгрузку роли реалма.
    * `isMigrateClientScopes` — включить в выгрузку области клиентов.
    * `isMigrateRealmGroups` — включить в выгрузку дерево групп.
    * `isMigrateFlows` — включить в выгрузку потоки аутентификации.
  * **Ответ:** `200 OK` — сущность `RealmRepresentation`.
  * **Особенность:** Автоматически вычищает ненужные массивы из итогового JSON в зависимости от переданных флагов для минимизации размера файла.


* **Импорт (`POST /configuration`)**
  * **Права:** `clients().requireManage()`
  * **Параметры URL:** 
    * `realm` (String, **обязательно**) — целевое имя рабочей области (создаваемой или обновляемой).
    * `isMigrateRealmRoles` — импортировать сразу роли реалма.
    * `isMigrateClientScopes` — импортировать сразу области клиентов.
    * `isMigrateRealmGroups` — импортировать сразу дерево групп.
    * `isMigrateFlows` — импортировать сразу потоки аутентификации.
    
  * **Тело запроса:** `RealmRepresentation`.
  * **Ответ:** `200 OK` — `RealmImportResponseDto` со статистикой по импортированным сущностям (количество ролей, скоупов, групп и потоков) и итоговой моделью реалма.
 
  * **Особенности (Smart Update):**
    * **Маппинг компонентов:** При создании нового реалма автоматически зануляет все `id` компонентов провайдеров. При обновлении существующего реалма — умным образом ищет существующие компоненты по `providerId` и подставляет их `id`, предотвращая ошибки дублирования.
    * **Частичный импорт:** Если выставлены флаги миграции связанных сущностей, плагин автоматически делегирует их импорт соответствующим сервисным слоям (`ImportRealmRolesService`, `ImportGroupsService` и т.д.) после сохранения самого реалма.
    * **Дефолтные группы:** Автоматически восстанавливает и переназначает дефолтные группы по их путям после завершения импорта основного дерева групп.
---
